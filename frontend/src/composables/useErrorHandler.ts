...
export function useErrorHandler() {
  ...
  function handleApiError(
    error: Error & { apiError?: boolean; response?: { data?: unknown; status?: number } },
  ...
  async function handleAsyncError<T>(
    ...
    try {
      ...
      handleApiError(error as Error & { apiError?: boolean; response?: { data?: unknown; status?: number } }, context, options)
      return null
    }
  }
  ...
  function withErrorHandling<T extends unknown[], R>(
    ...
    return async (...args: T): Promise<R | null> => {
      try {
        ...
        const context = errorContext ? { operation: errorContext } : undefined
        handleApiError(error as Error & { apiError?: boolean; response?: { data?: unknown; status?: number } }, context)
   ...
   * 创建重试函数
   */
  function createRetryHandler<T extends unknown[], R>(
    fn: (...args: T) => Promise<R>,
    maxRetries: number = 3,
    delay: number = 1000
  ) {
    return async (...args: T): Promise<R> => {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (attempt === maxRetries || !isRetryableError(error as Error & { response?: { status?: number } })) {
            throw error
          }
   ...
   * 判断错误是否可重试
   */
  function isRetryableError(error: Error & { response?: { status?: number } }): boolean {
    // 网络错误可重试
    if (!error.response) {
      return true
    }
